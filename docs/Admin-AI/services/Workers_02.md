# 🟦 51 — Workers_02
## Podman standalone handling (path detection; no dnf)
...
1. Publish the Podman invariant: standalone/static install; `dnf` does not manage Podman.
1. Define how automation locates Podman: configured absolute path OR deterministic detection.
1. Define refusal condition: if Podman binary is missing or managed unexpectedly, halt.
